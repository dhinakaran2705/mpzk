...
\item The interactive proof system is an $\round$-step protocol $\langle \Pi = \{\pi_i\}_{i \in [\round]}, \verifier \rangle$ where in every step $i \in [\round]$,  $\verifier$ broadcasts a challenge and  the output $m_i$ of an $\Num$-input function $\pi_i(\st^i_1, \ldots, \st^i_{\Num})$ is made available to a specific prover, acting on a special role of aggregator, who forwards the message to $\verifier$ in response to the challenge.   Here, $\st^i_j$ is the state of the prover $\prover_j$ at step $i$. $\st^1_j$ is set to $\prover_j$'s share of the witness $\wit_j$, randomness $r_j$, the public parameters $\sigma$ and updated later as the protocol proceeds and in particular include the public challenges of $\verifier$.  Computing $\pi_i$, without leaking the states to each other, may require interaction amongst the provers. It is important to note that a single prover, the aggregator, talks to the verifier on behalf of all the provers. This additional actor is key to achieve the property that $\verifier$ remains oblivious to the number of participant provers when the set of provers are honest and therefore there is no collusion between any prover and the verifier. The output of $\verifier$ is either an {\tt accept} (1) or a {\tt reject} (0).% Also, let $\tr$ be the transcript of the protocol.  and
\end{itemize}
