...
\]
\dnote{rewrite in terms of relation and with x in L as language}
Let $P_1, \ldots, P_N$ be $N$ parties. For the purpose of distributed proof generation, let $w = (w_1, \ldots, w_N)$ be an $N$-element witness set, with the parties possessing a partitioning of this set.\dnote{Change partition into additive sharing, with each share taking the input length of the circuit. A partition is a sub-class of sharing since the share for a prover can be its partition padded with zeros for the rest of input.} A DPZK protocol consists of three probabilistic polynomial time algorithms: $(\Setup, \Pi, V)$. 
\begin{itemize}
