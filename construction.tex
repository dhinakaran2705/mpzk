...
\begin{enumerate}
	\item \textit{P} sends $c$, the Merkle root of the commitment $C$, to \textit{V}.
	\item \textit{V $\rightarrow$ P}: $r\in_R \bbF^p$
	\item \textit{P}: $\widetilde{U}=\sum\lim\limits_{i\in[p]}r_iU[i]$, $\tilde{c}= \com(\widetilde{U})$
	\item \textit{P $\rightarrow$ V}: $\tilde{c}$
	\item \textit{V $\rightarrow$ P}: $\gamma \in_R \bbF^m$
	\item \textit{P}: $w= \gamma^T\widetilde{U}$
	\item \textit{P $\rightarrow$ V}: $w$
	\item \textit{V $\rightarrow$ P}: $Q\subset _R [n] : |Q|=t$
