...
\paragraph{\textbf{Proximity Check:}}\label{sec:proximity} In this check $\prover$ convinces $\verifier$ that $\cm_x$ is the commitment of a correct codeword matrix $\oracle_x$ which is encoding of a matrix $x$. If $\oracle_x$ is malformed, i.e. if it is sufficiently far away from a correct codeword matrix, then $\prover$ passes the following check with $\negl(t)$ probability.
	...
	\begin{theorem}\label{theo:1}
		For $e < \frac{d}{3} $, if $\prover(\cm, \cdot) \rightarrow \pi$ such that $\innp{\prover(\cm)}{\verifier(\pi)} $ is true, then there is an expected $\extrac^{\prover}(\cm) \rightarrow \oracle_x$ such that with overwhelming probability $\oracle_x$ satisfies $\com(\oracle_x) = \cm$ and $ d(\oracle_x, L^m) < e$  or it breaks the binding of the commitment.  
		\begin{comment}
	...
	\begin{theorem}
		For $e < \frac{d}{3} $ if $\innp{\prover(\cm, \calU, A, b)}{\verifier(\cm, A, b)} \rightarrow 1 $ then $\extrac^{\prover}(\cm) \rightarrow \oracle_x$ such that with overwhelming probability $\oracle_x$ satisfies $\com(\oracle_x)=\cm$ and $(d(\oracle_x,L^m) > e \text{ or } [ d(\oracle_x, l^m)\leq e \text{ and } x = \dec(\oracle_x)$ satisfies $ Ax = b]) $ or $\extrac$ breaks the binding of the commitment scheme. 
 	\begin{comment}	
	...
	\begin{theorem}
		For $ e < \frac{d}{3} $ if $\innp{P(\cm_x, \cm_y, \cm_z , \oracle_x, \oracle_y, \oracle_z)}{\cm_x, \cm_y, \cm_z}\rightarrow 1$ then $\extrac_P(\cm_x)\rightarrow \oracle_x$, $\extrac_P(\cm_y)\rightarrow \oracle_y$ and  $\extrac_P(\cm_z)\rightarrow \oracle_z$ satisfies 
		$$ \com(\oracle_x) = \cm_x, \com(\oracle_y) = \cm_y, \com(\oracle_z) = \cm_z$$ and
		$$ d(\oracle_x , L^m) > e \text{ or } d(\oracle_x , L^m)>e \text{ or } d(\oracle_x , L^m)>e$$ or
		$$ [d(\oracle_x,L^m) \leq e \text{ and } d(\oracle_y,L^m) \leq e \text{ and } d(\oracle_z,L^m) \leq e ]$$ $$\text{ and } x= \dec(\oracle_x),  y = \dec(\oracle_y), z=\dec(\oracle_x) \text{ satisfies } x\circ y = z$$ or
	...
	\begin{proof}
		...
		$\extrac$ emulates the role of the verifier and starts the protocol. In the step it cf
	\end{proof}
