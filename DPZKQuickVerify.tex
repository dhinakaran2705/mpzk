...
%Let $x\in \FF^N$ be the witness vector. 
Let $p,m$ and $\ell$ be integers such that $pm\ell\geq N$. 
%We choose ordered domains $G=\{\zeta_1,\ldots,\zeta_\ell\}$ and $H=\{\eta_1,\ldots,\eta_n\}$. 
...
%commitment space a group $\GG$ with independent generators $g_1,\ldots,g_m$ and $h$. 
Let $c_{ik} = \comm(\rsoracle[i,\cdot,k],\delta_{ik})$, $i\in [p]$, $k\in [n]$, where 
%$X[i,.,k]$ denotes the $m$-length vector $(X[i,1,k],\ldots,X[i,m,k])$ and 
