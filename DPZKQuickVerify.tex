...
where $\bm{c}=(c_1,\ldots,c_{s+\ell})$ and $\bar{P}$ is the submatrix of $P$
...
with $x=(1^m,0^{h-m})$, $v=r^Tb$ and $z=\beta P_0+\bar{P}\varphi$.
	\end{itemize}
...
\item $\Ag$ and $\verifier$ run the subprotocol:
	\begin{itemize}
	\item $b_{\rm 2d} =
\distproxTwoD(\FF,\GG,s+\ell,\dashL_1,\dashL_2,\bm{c};[[\bar{P}]])$
where $\bm{c}=(c_1,\ldots,c_{s+\ell})$ and $\bar{P}$ is the submatrix of $P$
consisting of the first $s+\ell$ columns. Here $\dashL_1=\rsc{\eta}{s+\ell}$ and
$\dashL_2=\rsc{\alpha}{2m}$.
	\end{itemize}
