...
\enc(\wit)$ and $\comoracle \gets \ocom(\ewit)$. The prover then sets the oracle 
...
[p]}R^i(\alpha_j,\cdot)Q^i(\alpha_j,\cdot)$ as $p_j$ (which will be of degree less than $s + \ell$), we can rewrite the expression as $\sum_{j}p_j(\zeta_k)=r^Tb$. Let $\Phi$ denote the $s\times (s+\ell)$ matrix such that $p_j(\zetabar)=\Phi \times p_j(\etabar)$ 

We define the matrix $P$ by $P[j,k]=p_j(\eta_k)$ for $j\in [h],k\in [n]$ which is a product codeword from $\rsc{\eta}{n,s+\ell} \otimes \rsc{\alpha}{h,2m}$ and fully determined by sub-matrix $\overline{P}$ consisting of the first $2m$ rows and
the first $s+\ell$ columns of $P$. Now our idea is to commit  $P$ using  commitment for product codeword and write the above expression as an inner-product argument involving $\overline{P}$. 
