...
To prove $\innp{x}{U[\cdot,k]}=v$, we first homomorphically compute commitment to $\overline{U}[\cdot,k]$ as \commentA{new notation$\langle\langle \rangle \rangle$. explain what it means. I don't get it. Is it $c_k$ the commitment for $\overline{U}[\cdot,k]$?}$\mathsf{cm}=\innph{\Lambda_1[k,\cdot]}{\bm{c}}$. Then we reformulate the inner product
 as 
...
$\mathsf{cm}=\sum_{k=1}^{s+\ell}\varphi_kc_k$ \commentA{shouldn't $\varphi_k$ be in the exponent? you have never defined the homomorphic operations and their notations on the commitments }. Using an inner-product argument
...
[p]}\rho_i\ewit[i,\cdot,\cdot]$. It then checks for all $u\in [t]$ \commentA{what is $t$} that $\sum_{a\in
