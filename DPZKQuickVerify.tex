...
$p(\etabar)=(\overline{P})^T[1^m]$ \pnote{dimension mismatch, it should be $\overline{P}^T[{1^m}||{0^m}]^T$}, and thus
...
matrix $\Lambda_3$ (see Section ~\ref{sec:codesandmatrices}) as $\mathsf{cm}_u=\sum_{a\in [s+\ell]}\Lambda_3[a,k_u]c_k$. \pnote{Check the equation. It should be $\cm_k = \sum_{a\in[s+\ell]} \Lambda_3^T[a,k] c_a$ } Next,
