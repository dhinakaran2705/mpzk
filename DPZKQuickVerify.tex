...
\subsection{Codes, Matrices and Commitments}\label{sec:codesandmatrices}
...
and $\mc{C}_2 :=\cic{L_2}{n}$ denote the interleaved codes of $L_1$ and $L_2$. We denote the
...
as discussed in Section \ref{sec:prelims}. We now discuss the commitment scheme
for matrices in $L_1\otimes L_2$. Note that a matrix $U\in L_1\otimes L_2$ is
...
to Section ~\ref{sec:codesandmatrices}, instead of
...
commitments to columns of $\overline{P}$. Let $\Lambda_1$ denote $n\times
(s+\ell)$ matrix that
maps the first $s+\ell$ entries of a row of $P$ to entire corresponding row of $P$. Similarly let
$\Lambda_2$ denote the $h\times 2m$ matrix which maps the first $2m$ entries of
a column of $P$ to entire corresponding column of $P$. Note that the matrices exist as the
operations are linear transformations.\smallskip
...
$j_u$. The verifier checks the following inner products:
\begin{enumerate}[{\rm (i)}]
...
matrix $\Lambda_1$ as $\mathsf{cm}_u=\sum_{a\in [s+\ell]}\Lambda_1[a,k_u]c_k$. Next,
we observe that $\innp{f_u}{P[\cdot,k_u]}=\innp{f_u}{\Lambda_2
Z_u}=\innp{f_u^T\Lambda_2}{Z_u}$. In
...
$W_{iu}=\Omega_2V_{iu}$ where $V_{iu}$ denotes the vector consisting of the
first $m$ entries of $W_{iu}$. We rewrite the preceeding inner product as
$\innp{f_u^T\Omega_2}{V_{iu}}=X_u[i]$, which can be executed as the commitment
...
encoding. To check that, the verifier initially (before sending the query $Q$) sends a vector $\rho\sample
