...
%(Section ~\ref{sec:matrixcommitment})  
where only 
...
\noindent{\em Consistency of oracle $\pi$ and $P$ via $\ewit$}: The verifier additionally
...
$u\in [t]$.  For $u\in [t]$, let %\commentA{changed this notation. change in other places}$
...
$[t]$, with $c_{k_u}$ (which can be computed from $c_1,\ldots,c_{s+\ell}$, as in Section~\ref{sec:matrixcommitment})  as the commitment of  $P[\cdot,k_u]$. %It is readily checked that for correct $P$ and  $X_u$, the identity holds. 
%The inner-product can be checked with the commitment of  $P[\cdot,k_u]$, $c_{k_u}$,  which can be computed from $c_1,\ldots,c_{s+\ell}$, as in Section~\ref{sec:matrixcommitment}.
...
%$\innp{\bm{1}^T_{j_u} \Lambda_{h,m}}{\ewit[i,\cdot,k_u]^T\Lambda_{h,m}}=X_u[i]$ 
 %Here $W_{iu}$ denotes the vector $\ewit[i,\cdot,k_u]$. 
%Again,  the inner-products can be verified as in Section ~\ref{sec:matrixcommitment}.
...
%\noindent{\bf Remark}: \commentA{it does not make any sense to me? what's the point?}When the simulator rewinds the transcript $\mathsf{tr}$, and starts with fresh randomness for the verifier, it gets the prover message as response if the randomness was used as the challenge to the prover, and it gets oracle response if the randomness was used as query to the oracle.

We give a proof-sketch  in Appendix ~\ref{app:linear_soundness}.
...
\FF$. $\Sim$ picks $O\sample \FF^{p\times t}$, and computes $\tilde{\ewit}[\cdot,
k_u] = \sum_{i\in[p]} \rho_i \ewit[i,\cdot, k_u]$, $\tilde{O}[k_u]= \sum_{i\in[p]}
\rho_i O[i,u]$, and $\tilde{c}_{k_u}  \leftarrow\\ \comm(\tilde{\ewit}'[\cdot, k_u] ,
\tilde{O}[k_u] )$ for all $u\in[t]$ and $\pi[i,k_u] \leftarrow \comm(\ewit'[i,\cdot,k_u],
...
\end{lemma}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Zero knowledge for Quadratic check
The verifier's view for the $\quadcheck$ protocol consists  of:
\begin{itemize}
...
\noindent{\bf Distributed Quadratic Check}: Here the distributed variant requires an additional interaction among the provers. Recall that in response to $\verifier$'s challenge $r\in \FF^p$, the provers need to compute $P$ as:
\begin{align*}
P[j,k] & =\sum_{i\in
...
\begin{lemma}\label{lem:distlincheckzk}
	For $\xi\in [\Num]$, let $\View^\xi_\Ag$ denote the view of  $\Ag$ in the distributed
	linear check protocol consisting of messages from \ $\distprover$ with
...
Thus, the view of $\Ag$ can be perfectly simulated.
\end{proof}
