...
\commentA{this is unnecessary notation. we have $d_1$ and $d_2$ for it. let's remove this and makes changes accordingly.}For $U^\ast\in \mc{M}_{p,h,n}$ define $d(U^\ast,\mc{W}_i)=\min\{\Delta_i(U^\ast,U):U\in \mc{W}_i\}$ for $i=1,2$.
...
$\innp{*}{\overline{P}\times\varphi} = r^Tb$, where $*$ and $\varphi$ are public vectors 
 and so the commitment for $\overline{P}\times\varphi$ can
...
\subsection{Reudcing MPC Overhead for Small Shared Circuits}
...
case, when each input wire is ``assigned'' to a unique prover. We call a wire
$i$, for $i\in [N]$ to be {\em shared} if its value depends on input wires from
more than one prover, otherwise we call it {\em non-shared}. 
Let $N_s$ denote the number of shared wires. Without loss
of generality, we may assume that $\wit_s=(w_1,\ldots,w_{N_s})$ corresponds to values of
the shared wires. We write $\wit=[\wit_s||\wit_{ns}]$, where $\wit_s$ and
$\wit_{ns}$
represent shared and non-shared wires respectively. {\color{blue} To complete:}
