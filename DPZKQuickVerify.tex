...
\commentA{now complete and derive the above inner product form} \commentA{I tried to avoid p poly altogether as it is unnecessarily brought in the picture. try to complete the explanation without brining p in the picture. }. 
...
Let $\Phi$ be a $s\times (s+\ell)$ matrix such that $[p_j(\zeta_1), \ldots, p_j(\zeta_s)]^T = \Phi \times [p_j(\zeta_1), \ldots, p_j(\zeta_s)]^T \forall j\in[h]$ %$p(\zetabar)^T = \Phi \times p(\etabar)^T$. \\
{\footnotesize
\begin{align*}
...
\begin{proof}
	...
	%\begin{itemize}
		...
		%share of $0^h$.
		uniform distribution on the set $\{(\langle{z}\rangle^1,\ldots,\langle{z}\rangle^{\Num})\in (\FF^{2m})^{\Num}: z=\sum_{\xi\in[\Num]}\shr{z} \text{ and } \sum_{j\in[m]} z[j]=0 \}$. %$P_0$ of length $2m$ satisfying $\sum_{j\in [m]}P_0[j]=0$ 
		Therefore, jointly all the provers have the correct witness and individually it does not reveal any information due to padding with $0^{2m}$.
