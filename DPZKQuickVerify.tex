...
\begin{proof}
	...
	%\begin{itemize}
		...
		%share of $0^h$.
		uniform distribution on the set $\{(\langle{z}\rangle^1,\ldots,\langle{z}\rangle^{\Num})\in (\FF^{2m-1})^{\Num}: z=\sum_{\xi\in[\Num]}\shr{z} \text{ and } \sum_{j\in[m]} z[j]=0 \}$. %$P_0$ of length $2m$ satisfying $\sum_{j\in [m]}P_0[j]=0$ 
