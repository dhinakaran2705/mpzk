...
\paragraph{Privacy among the provers: } In the oracle construction phase, provers will interact to get the share of zero. This is required to achieve the hiding of the polynomials send by the provers to the aggregator. n single prover case the verifier is not learning anything about the polynomials constructed by the prover in the $\linearcheckTwoD$ and $\quadcheckTwoD$ because, $q^{lin}(\cdot)$ is a random polynomial which add up to zero when evaluated on $\bm{\zeta}$ and $q^{quad}(\cdot)$ is a random polynomial which evaluates to zero for all $\zeta \in \bm{\zeta}$. But simply blinding the same way for each of the provers in multi prover setting is not enough to hide the distribution of the polynomials from the aggregator, since shares of individual parties may not satisfy the constraints(linear and quadratic), in that case hiding by random polynomials which adds up to zero when evaluated on $\bm{\zeta}$(for linear check, similar for quadratic but the random polynomials which evaluates to zero for all $\zeta \in \bm{\zeta}$) may leak additional informations. To avoid that, for both $p_{\nu}^{lin}(\cdot)$ and $p_{\nu}^{quad}(\cdot)$ each prover, in addition to corresponding hiding polynomials add shares of zero to each coefficient, so that all the polynomials have the same distribution as random polynomial, and when the aggregator adds the polynomials, the aggregated polynomial is correct and random from the appropriate set of polynomials.
