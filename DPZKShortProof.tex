...
such as Bulletproofs \cite{bulletproofs}, Spartan \cite{spartan} to the\pnote{ are we describing about DP of spartan?}
...
$A\wit\circ B\wit=C\wit$, where $\bm{x} \circ \bm{y} = (x_1\cdot y_1, \cdot, x_n\cdot y_n)$ for any $n$-length vectors $\bm{x}$ and $\bm{y}$. \commentA{define the operation $\circ$}. We follow the broad outline of the interactive PCP
construction in \cite{ligero} which includes: (i) extending the witness, denoted as extended witness, to the concatenation of bits over all the wires in topological order of a Boolean circuit \pnote{Boolean or arithmetic circuit?} representing the statement   (ii) encoding the extended witness via suitable
