...
using the ``BCS'' transformation in \cite{BCS16} \commentA{BCS transformation in one line}.  \commentA{this part should move to the DPZK part}To serve as a
...
$A\wit\circ B\wit=C\wit$, where $\bm{x} \circ \bm{y} = (x_1\cdot y_1, \cdot, x_n\cdot y_n)$ for any $n$-length vectors $\bm{x}$ and $\bm{y}$. R1CS circuits are equivalent to Arithmetic circuits, to avoid additional linear check for the addition gates, we are considering R1CS circuits for our protocol, with one additional linear check, one can easily modify our protocol for arbitrary arithmetic circuits. \commentA{add one line why R1CS is better than arithmetic circuit formulation} We follow the broad outline of the interactive PCP
...
argument size of $O(N^{1/c})$, the verifier \commentA{keep away aggregator from SPZK. report aggregator's complexity in the DPZK section latter} incurs $O(N^{1-2/c})$
