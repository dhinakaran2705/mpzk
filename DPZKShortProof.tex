...
$A\wit\circ B\wit=C\wit$, where $\bm{x} \circ \bm{y} = (x_1\cdot y_1, \cdot, x_n\cdot y_n)$ for any $n$-length vectors $\bm{x}$ and $\bm{y}$. \commentA{add one line why R1CS is better than arithmetic circuit formulation} We follow the broad outline of the interactive PCP
