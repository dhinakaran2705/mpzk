...
\item $\verifier\rightarrow\prover$: The verifier sends a random $r\sample \bbF^\circsize$ to the prover.

\item Both $\verifier$ and $\prover$ compute $\sfR = r^T A$, then interpolate polynomials $R_i:i\in [m]$ in the evaluation domain $G$ such that $R_i$ interpolates the vector $(\sfR[(i-1)n+1],\ldots,\sfR[i \cdot n])$ for $i\in [m]$. 

\item $\prover\rightarrow\verifier$: The prover computes polynomial  $p(\cdot)=\sum_{i\in [m]}R_i(\cdot)\hat{f}^x_{i}(\cdot)$ and sends $p$ to the verifier.
...
\item $\comoracle \rightarrow \verifier$: The verifier queries the commitment oracle $\comoracle$ on $\{j_u\}_{u \in [t]}$.

\item $\prover\leftrightarrow\verifier$: The verifier wants to check that $\sum_{i\in [m]} R_i(\eta_{j_u}) \cdot \rsoracle[i, j_u] = p(\eta_{j_u})$ for each $u \in [t]$ with $\rsoracle[\cdot, j_u]$ being the opening of $\comoracle[j_u]$.
This is the same as verifying the inner product $\innp{\hat{R}_{j_u}}{\rsoracle[\cdot,j_u]} = p(\eta_{j_u})$, where $\hat{R}_{j_u} = (R_1(\eta_{j_u}), \ldots, R_m(\eta_{j_u}))$ and $\rsoracle[\cdot, j_u]$ denotes the $m$-length vector $(\rsoracle[1, j_u], \ldots, \rsoracle[m, j_u])$.
To do so, the prover and the verifier run an inner-product argument for each $u \in [t]$. The verifier proceeds if the arguments succeed for all $u \in [t]$, else rejects.

\item The verifier also checks that $\sum_{k\in [\ell]}p(\zeta_k)=0$ and accepts if the above check succeeds, else rejects.
\end{enumerate}
...
\item $\comoracle \rightarrow \verifier$: The verifier queries the commitment oracle $\comoracle_x$, $\comoracle_y$ and $\comoracle_z$ on $\{j_u\}_{u \in [t]}$.

\item $\prover\leftrightarrow\verifier$: The verifier wants to check that 
$\sum_{i\in [m]} r_i(\rsoracle_x[i,j_u] \cdot \rsoracle_y[i,j_u]-\rsoracle_z[i,j_u] = p(\eta_{j_u})$ for each $u \in [t]$ and for the $\rsoracle$ vectors corresponding to the commitments queried from the oracle.
This is equivalent to the checking $\innp{r \circ \rsoracle_x[\cdot,j_u]}{\rsoracle_y[\cdot,j_u]} - \innp{r}{\rsoracle_z[\cdot,j_u]} = p(\eta_{j_u})$. There are two inner products in the above statement and the prover does not want to reveal the values of the individual inner products. Hence, \name2D{} combines them in to a single inner product relation.
...
\]
To facilitate this, $\comoracle_x,\comoracle_y$ and $\comoracle_z$ should have been committed with distinct and independently chosen sets of generators.\footnote{By independent, we mean that the prover does not know the Diffie-Hellman relation between any pair of generators.} And, the set of generators used for committing $r$ (to be concatenated with $r \circ \rsoracle_x[\cdot,j_u])$) should also be independent of the above three sets of generators.
The verifier proceeds if the argument succeeds for all $u \in [t]$, else rejects.

\item The verifier also checks that $\sum_{k\in [\ell]}p(\zeta_k)=0$ and accepts if it succeeds, else rejects.
\end{enumerate}
...
\begin{enumerate}[{\rm 1.}]
\item $\verifier\rightarrow \prover$: The verifier samples $\gamma \sample \bbF^n$ and sends to the prover.

\item $\prover\rightarrow \verifier$: The prover computes $w=\gamma^T U$ and sends it to the verifier. 

\item $\verifier\rightarrow \prover$: The verifier sends $t$ randomly sampled indexes $\{j_u\}_{u\in [t]}$ from $[n]$ to the prover.

\item $\comoracle \rightarrow \verifier$: The verifier queries the commitment oracle $\comoracle$ on $\{j_u\}_{u \in [t]}$.

\item $\prover\leftrightarrow\verifier$: The verifier now wants to verify that $w_{j_u} = \gamma^T U[\cdot, j_u]$, for all $u \in [t]$. To do this, the prover and the verifier will run the inner-product argument to check $\innp{\gamma}{U[\cdot, j_u]} = w_{j_u}$ for all $u \in [t]$. The verifier proceeds if the checks succeed, else rejects.

\item The verifier also checks if  $w\in L$ and accepts if it succeeds, else rejects.
\end{enumerate}
