...
We use $\sample$ to denote drawing from a distribution uniformly at random.  \commentA{added this; please replace  $\sample$ with $\gets$ in all the appropriate contexts. I am doing it on my way; but I do not think I can make a pass on the whole paper; so do in your respective sections}
...
$\rsc{\eta}{n,k}$ \commentA{I changed the notation, it is hard to remember $n$ otherwise} to denote the Reed Solomon code with
...
$p\times m\times n$ matrix such that $d_1(U^\ast,\mc{C}^p)>e$.  \commentA{in the following $u_0$ cannot be any $m\times n$ matrix  for the proposition to be true. check. it should be from $\mc{C}$ or with the same feature as that of a slice}\pnote{I cross checked with Nitin, statement of the proposition is correct.} Then for any
...
We define an interactive protocol that allows proving inner-product relation over committed vectors, where the commitment corresponds to a non-interactive perfectly hiding and computationally binding commitment scheme, with an additional property of homomorphicity.  \commentA{removed ref to commitment scheme. it's standard. comment out from appendix}\pnote{Done} We use Pedersen vector commitment scheme $\comm$ with message space $\FF^m$,
...
randomness $r\in \FF$ is given by $\comm(x,r)=h^r \prod_{i=1}^m (g_i)^{x_i} $.  This commitment is homomorphic and let $\calC$ determines commitment space.
...
We now define the language for inner-product w.r.t. the above  as $\calL \subseteq \FF^m \times \calC \times \FF$ as:
\begin{equation*}
%\begin{aligned}[c]
\begin{array}{l}
\calL_\sigma = \{(x,c,z):\exists \; y,r \text{ s.t. } 
...
%The $\NP$ relation $\calR_\sigma$ for the language $\calL_\sigma$ consists of pairs $(\stmt,\wit)$ with $\stmt=(c_1,c_2,v)$ and $\wit=(x_1,x_2,r_1,r_2)$ such that $c_1=\com(x_1,r_1)$, $c_2=\com(x_2,r_2)$ and $\innp{x_1}{x_2}=v$.
 Interestingly, the inner-product argument is used to save on communication. Proving  an inner-product statement requires sending $y$ on clear which costs more than  the argument-size. Our language for inner-product is a simpler version of those defined in \cite{}, where the first argument is also hidden in a commitment. 
\begin{definition}[Inner-product Argument \cite{InnerProductDLS,bulletproofs} \commentA{fill in the ref}]\label{defn:innerproductarg} 
