...
Let $L_1 := \rsc{\eta}{\ell}$ and $L_2 := \rsc{\alpha}{m}$ \commentA{include the length of codeword $n$ and $h$ in the argument i.e. $\rsc{\eta}{n,\ell}$} be $[n,\ell]$ and
...
$d_2(U^\ast,\mc{C}_2)<e_2$ for $e_1\leq n-\ell$ and $e_2\leq h-m$\commentA{what are $d_1$ and $d_2$? These are never defined. can you simplify notation; remove $e_1$ and $e_2$ }. Then, there exists $U\in L_1\otimes L_2$ and sets
