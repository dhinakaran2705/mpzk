...
$\mc{C}$ as an $[n,k,d]$ code over the field $\HH$, where $\HH\cong \FF^m$. %We elaborate on this correspondence in Appendix \ref{app:AltILC}.
...
The proof will be given in full version.
...
we will we a variant of inner product where one of the vector is public and the other one is private. $\innerproduct(\pp, x, \cm, v ; z)$ is the notation we will be using through out the paper, where $x$ is a public vector and $\cm$ is the commitment of the private vector $z$ such that $\innp{x}{z} = v$.
