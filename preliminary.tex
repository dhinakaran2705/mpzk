...
$p\times m\times n$ matrix such that $d_1(U^\ast,\mc{C}^p)>e$.  \commentA{in the following $u_0$ cannot be any $m\times n$ matrix  for the proposition to be true. check. it should be from $\mc{C}$ or with the same feature as that of a slice} Then for any
...
We define an interactive protocol that allows proving inner-product relation over committed values, where the commitment corresponds to a non-interactive perfectly hiding and computationally binding commitment scheme defined in Appendix \ref{app:DefCommitment}. %We start with a definition of such a commitment scheme. 
We will assume that all the message spaces $\calM_\sigma$ output by the $\gen$ algorithm come equipped with an inner-product operator $\innp{\cdot}{\cdot}:\calM_\sigma\times \calM_\sigma\rightarrow Z$. We define the language $\calL_\sigma\subseteq \calC_\sigma\times \calC_\sigma\times Z$ as \commentA{change this language if needed; all cases I encountered so far didn't need the first commitment which is a public value}:
\begin{equation*}
%\begin{aligned}[c]
\begin{array}{l}
...
\begin{definition}[Inner-product Argument \cite{} \commentA{fill in the ref}]\label{defn:innerproductarg} 
\commentA{say why inner-product argument does not need privacy. It seems it improves proof-sise and performs better than sending the witness on clear}
...
We mention some instantiations of  commitment schemes and corresponding inner-product arguments in Appendix~\ref{inner-product-instan}. 
