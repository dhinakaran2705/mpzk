	...
	{\footnotesize
		...
		\begin{framed}
			...
			\begin{enumerate}[{\rm 1.}]
				...
				\item $\verifier\rightarrow\prover$: $Q=\{(j_u,k_u):u\in [t]\}$ for %$(j_u,k_u)
				$Q\sample [h]\times [n]$ for $u\in [t]$. And $\tau \sample \FF^s$, $\gamma \sample \FF^m$.
				\item $\verifier\rightarrow\pi$: $\{k_u:u\in [t]\}$.
				...
				\begin{enumerate}
					...
					\item
					$\innerproduct(\mathsf{pp},\bm{1}_{j_u}^T\Lambda_{h,m},T_u,\innp{\delta}{T_u};W_u[1:m])$ (consistency of $X_u, Y_u, Z_u$ with $\pi$). 
				\end{enumerate}
	...
	{\footnotesize
		...
		\begin{framed}
			...
			\begin{enumerate}[{\rm 1.}]
				...
				\item  {$\Ag$ computes $z=\sum_{\xi\in[\Num]} \shr{z}$}.
	...
	{\footnotesize
		...
		\begin{framed}
			...
			\begin{enumerate}[{\rm 1.}]
				...
				\item  {$\Ag$ computes $z=\sum_{\xi\in[\Num]} \shr{z}$} and $W_u = \sum_{\xi\in[\Num]} \shr{W_u}$.
...
\subsection{Proof of soundness of Linear Check}\label{app:linear_soundness}
...
\subsection{Performance Parameters}\label{sec:performance}
...
argument size $\zkcomm=n+6\ell+4s+4mt+5t-5$, prover complexity
 $t_\prover=O(N)\cfop+4m(\cfft(s)+\cfft(n))$, verifier complexity
$t_\verifier=O(N)+4m\cfft(s)$, and soundness error
$\kappa_{lg}=(1-e/n)^t+5((2\ell+e)/n)^t$.\smallskip

\noindent{\bf Bulletproofs}: The performance parameters for the Bulletproofs
protocol for circuit with $N$ wires is given by: argument size
$\zkcomm=2\log(n)+13$, prover complexity $t_\prover=9N\cexp +
2\cmexp(2N)+3\cmexp(N)$, verifier complexity $t_\verifier=N\cexp + \cmexp(2n)$,
and soundness error $\kappa_{bp}=N/|\FF|$.

For Ligero, we set $n=8\ell$,  to achieve soundness of $2^{-\secpar}$ with roughly
 $t=5\secpar/2$.
