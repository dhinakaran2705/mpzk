...
\pdnote{do we want to also cite bulletproof - distributed range proof}
...
In the single-prover setting, the efficiency of a proof generation algorithm is usually measured in terms of the number of arithmetic operations i.e., the \textit{proof generation computation complexity} ($\prcomm$). The number of \textit{zero-knowledge rounds ($\zkrounds$)} of interaction between the prover and the verifier is also considered an efficiency parameter when a non-interactive prover is obtained from its interactive version in a provably secure manner \cite{BCS16}. But, these parameters do not capture the core bottleneck in the setting of multiple provers. 
For instance, Spartan \cite{spartan} has an $O(N \log N)$ proof generation algorithm, but due to the high multiplicative complexity of proof generation its straightforward distributed version requires $O(N^2)$ MPC multiplications where $N$ is the number of gates in the circuit representation of the predicate being proved. %Similar arguments will be made in a future section for the other state-of-art protocols like Aurora \cite{aurora}, Bulletproofs \cite{bulletproofs}. \dnote{mention the asymptotics in the above claims}
...
\name{} is an IPCP protocol that follows the outline of Ligero: (i) encoding the witness via a suitable error correcting code, (ii) checking linear relations on the witness, (iii) checking quadratic relations on the witness, and (iv) a proximity protocol to check that the witness is correctly encoded. The witness encoding is committed as an ``oracle'' and is queried during the linear, quadratic and the proximity checks. The core technical contribution of our work is a novel way to encode the witness and thereby design the other parts of the zero-knowledge protocol around the new encoding. This encoding first represents the witness as a \textit{3D matrix} of dimensions $p \times m \times n$ whose product is $O(N)$. We regulate the three dimensions such that we get \textit{sub-linear} complexities in both the communication between the provers $\prcomm$ and the proof size $\zkcomm$. 
%A brief overview follows. To construct the oracle, we first apply a 2D product Reed-Solomon code to  every $1 \times m \times n$ \textit{slice} of the 3D witness matrix. Then every column (second dimension) is committed using a
  We then design 
 skew the 3D matrix

The first step towards achieving a $\DPZK$ is to ensure that oracle does not reveal the number of provers. We do so by committing the witness encoding using a homomorphic commitment.
aggregation w/ zk - homomorphic oracles
zkComm - 3D
