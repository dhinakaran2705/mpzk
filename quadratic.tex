...
\noindent{\em Consistency of $P$ and $p(\overline{\bm{\zeta}})$:} Same as the linear check, only differences are here $p(\cdot) = \sum_{j\in[m]}p_j(\cdot)$ is a polynomial of degree $2\ell$, $\Phi$ is a matrix that takes $p(\etabar)$ to $p(\zetabar)$, where $p(\etabar) = [p(\eta_1),\ldots, p(\eta_{2\ell})]$ and $p(\zetabar)$ is same as linear check. Here, Finally inner product reduces to $\innp{[\gamma||0^m]^T}{z}$, where $z= beta P_0 + \overline{P}\times \varphi$ and $P_0\in \FF^{2m}$ is a random vector such that $P_0[j]=0 \forall j\in [m]$. Commitment of $P_0$ is $c_0$ is sent to the verifier apriori. Therefore, commitment to $z$ is $\cm = c_0^{\beta}\prod_{k=1}^{2\ell}(c_k)^{\varphi_k}$.
