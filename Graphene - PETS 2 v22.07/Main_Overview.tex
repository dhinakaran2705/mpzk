...
There is known a transformation that converts ZK designed with IPCP to an interactive ZK via collision-resistant hash functions \cite{IshaiMS12, IshaiW14} which in turn can be made non-interactive  \cite{FS86, BCS16}. 
...
\begin{table*}[htp] 
	...
	\resizebox{\textwidth}{!}{
		...
		\begin{tabular}{|c|c|c|c|c|c|c|} 
			...
			\hline
			D-Ligero & $O(\sqrt{N})$ & $O(1)$ & $O(N) E+ O(N\log (N))M$ &$O(N)E+ O(N)M$ & $2$ & $ O(\Num \cdot N + \Num^2\cdot \max(N_s, \sqrt{N}))$\\
			\hline
			D-Ligero++ & $O(\log^2{(N)})$ & $O(\log (N))$ & $O(N\log(N)) E+ O(N\log (N))M$ &$O(N\log (N))E+ O(N)M$ & $\log (N)$ & $ O((\Num\cdot N + \Num^2\cdot N)\log (N))$\\
			\hline
			D-Aurora & $O(\log^2{(N)})$ & $O(\log (N))$ & $O(N\log (N)) E+ O(N\log (N))M$ &$O(N\log(N))E+ O(N)M$ & $\log (N)$ & $ O((\Num\cdot N + \Num^2\cdot N)\log (N))$\\
			\hline
			D-Bulletproofs & $O(\log (N))$ & $O(\log(N))$ & $O(N)E$ & $O(N)E$ & 2 & $ O(\Num\cdot N + \Num^2\cdot N_s)$ \\
			\hline
			\name & $O(N^{1/c})$ & $O(\log(N))$ & $O(\frac{N}{\log(N)})E + O(N\log(N))M$ & $O(N^{1-2/c})E + O(N)M$ & 2 & $ O(\Num\cdot N^{1-2/c} + \Num^2\cdot \max(N_s,N^{1-2/c}))$ \\
			\hline
