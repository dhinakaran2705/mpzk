...
information to the verifier. In this work, we the study the notion of distributed proof
...
amount of communication $\prcomm$ among the provers, (iii) we propose a compiler that converts Interactive Oracle Proofs (IOP)-based ZK protocols into a $\DPZK$ protocol, (iv) we present a new single-prover zero-knowledge protocol \name{} in the Interactive Probabilistically Checkable Proofs (IPCP) paradigm which admits $O(N^{1/c})$
...
of $O(\Num \cdot N^{1-2/c}+ \Num^2\cdot \max(N_s,N^{1-2/c}))$  and  a round complexity $\prrounds $ of 2, where $N_s$ denotes the number of multiplication gates that takes witnesses of more than one provers,  in the arithmetic circuit representing the prover's computation in \name{}. This is achieved by compiling a Ligero-style protocol followed by several optimizations.}  
\end{abstract}
%=======
