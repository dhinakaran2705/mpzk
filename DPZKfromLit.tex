...
\begin{center}\label{tab:SPZK}
	...
	\begin{tabular}{|c|c|c|c|c||}
		...
		\hline
		Prover communication& MPC($n^2,1$) + $\Num ?$& MPC($n,1$) + $\Num n$& MPC($n,1$) + $\Num n^{1-1/c}$& MPC($n,1$) + $\Num n^{1-1/c}$\\
		\hline 
		\# of MPC & $\log(n)$& 1 & 1 & 1 \\
		\hline
		% &computation & communi- cation & MPC rounds& & & \\ 
...
Here $n$ is the size of the circuit and, $c$ is a positive integer of our choice. $\Num$ in the number provers in the $\DPZK$ setting. 
