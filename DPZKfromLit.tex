...
In \cite{bulletproofs}, these above checks are reduced to a single inner product argument, in the following way: 
Prover commits to the input vectors $\bm{a}_L$ and $\bm{a}_R$ and output vector $\bm{a}_O$ and computes $A_I$ and $A_O$ respectively. Prover sends $A_I$ and $A_O$ to the verifier.
Also, prover picks $s_L$ and $s_R$ uniformly at random and computes the commitment $S$ and sends $S$ to the verifier.
Verifier gives random challenges $y, z$ from $\ZZ^\ast_p$ to the prover.
Then prover computes vector polynomials $l(X)$ and $r(X)$ using $\bm{a}_L, \bm{a}_R, \bm{a}_O, s_L, s_R, y, z$ and other public vectors.
Prover also computes a polynomial $t(X) = \innp{l(X)}{r(X)}$. And the construction of $l(X)$ and $r(X)$ is such that the co-efficient of $X^2$ in $t(X)$ is independent of $\bm{a}_L, \bm{a}_R, \bm{a}_O, s_L, s_R$, which can be computed by the verifier, if $\bm{a}_L, \bm{a}_R, \bm{a}_O$ satisfy the Hadamard-product relation and linear constraints. To check this, prover commits to all the co-efficients of $t(X)$ other than co-efficient of $X^2$, and verifier gives a random point $x$ to evaluate $\bm{l} = l(x)$ and $\bm{r} = r(x)$. Which reduces to an inner product check whose witness is $\bm{l}$ and $\bm{r}$. 

We are giving a DPZK version of Bulletproofs, where $\distprover$ starts the protocol with $\shr{\bm{a}_L}, \shr{\bm{a}_R}, \shr{\bm{a}_O}$ $\forall \xi \in [\Num]$ such that $\sum_{\xi\in [\Num]} \shr{\bm{a}_L} = \bm{a}_L$, $\sum_{\xi\in [\Num]} \shr{\bm{a}_R} = \bm{a}_R$, $\sum_{\xi\in [\Num]} \shr{\bm{a}_O} = \bm{a}_O$. $\Ag$ is an aggregator.
$\distprover$ computes the commitment $\shr{A_I}, \shr{A_O}, \shr{S}$ and sends these values to $\Ag$. $\Ag$ combines and gets $A_I$, $A_O$ and $S$, due to the homomorphic property of the commitment scheme, and sends these values to the verifier.
...
$\distprover$ commits to all the coefficients of $\shr{t}(X)$ other than the coefficient of $X^2$. Sends these committed values to $\Ag$. $\Ag$ combines and sends them to the verifier.
