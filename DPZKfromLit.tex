...
%\begin{center}
	...
	\begin{tabular}{|c|c|c|c|}
		...
		\midrule
		Argument size & $N^{1/c}$ & $\log (N)$ & $N^{1/c}$\\
		\midrule
		Rounds(ZK) & $\log (N)$ & $\log (N)$  & $\log (N)$\\
		\midrule
		Prover's complexity & $O(N)E$ + $O(N\log(N))M$ & $O(N)E$ & $O(\frac{N}{\log(N)})E + O(N\log(N))M$ \\
		\midrule
		Verifier'c Complexity& $O(N^{1-1/c})E$\tnote{$\ast$} & $O(N)E$ & $O(N^{1-2/c})E + O(N)M$\\
		\midrule
		Prover communication& MPC($N_s^2,1$) + $\Num N$\tnote{$\ast\ast$} & MPC($N_s,1$) + $\Num N$& MPC($N_s,1$) + $\Num N^{1-1/c}$\\
		\midrule
		Number of MPC & $\log(N)$& 1 & 1 \\
		\bottomrule
		\end{tabular} 
	\begin{tablenotes}
